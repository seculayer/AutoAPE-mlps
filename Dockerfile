...
ARG APP_DIR="/opt/app"
ARG CLOUD_AI_DIR="/eyeCloudAI/app/ape/mlps/"
...
COPY --chown=aiuser:aiuser mlps ${CLOUD_AI_DIR}/mlps
COPY --chown=aiuser:aiuser mlps.sh ${CLOUD_AI_DIR}
RUN chmod +x ${CLOUD_AI_DIR}/mlps.sh

ENV PATH="${CLOUD_AI_DIR}/.venv/bin:$PATH"

CMD ["bash", "-c", "${CLOUD_AI_DIR}/mlps.sh"]
